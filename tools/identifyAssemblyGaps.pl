...
identifyAssemblyGaps.pl - Generate a BED of gaps from a haploid assembly FASTA

=head1 SYNOPSIS

identifyAssemblyGaps.pl [options]
...
This script generates a BED of gap intervals from an input haploid genome
assembly FASTA.
...
# identify the gaps for that scaffold. The code is slightly messier, but
...
while (my $line = <$genome_fh>) {
   ...
   # output the gaps from the previous scaffold (since we're on
