...
all_lens <- read.table(input_file,
                       colClasses=c("character", "integer", "integer"),
                       col.names=c("Assembly", "Index", "Length"),
                       header=FALSE)
...
contiguity_plot <- all_lens %>%
   mutate(Assembly=factor(Assembly,
                          levels=Assembly_levels,
                          labels=Assembly_labels)) %>%
   group_by(Assembly) %>%
   mutate(CumulativeLength=cumsum(Length)/1e6) %>%
...
#            aspect.ratio=0.8,
      ...
      labs(x=expression(paste("Contig Rank (", log[10], " scale)")),
           y="Cumulative Assembly Length (Mbp)",
           colour="",
           title="Assembly Contiguity")

ggsave(plot_file,
       plot=contiguity_plot,
       dpi=500,
       units="cm",
       width=16.0,
       height=12.0)
