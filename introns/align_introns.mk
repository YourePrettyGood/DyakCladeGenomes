...
trimmed_aligned_all/%_trimmed.fasta : trimmed_aligned_refs/%_aligned.fasta trimmed_unaligned_samples/%_trimmed.fasta trimmed_aligned_all
	addAlignedGaps.pl -a <(fasta_formatter -i $(word 1,$^)) -i $(word 2,$^) 2> logs/addAlignedGaps_$*.stderr | trimAlignment.awk > $@
...
trimmed_unaligned_samples/OG%_trimmed.fasta : raw_introns_samples/OG%_unwrapped.fasta trimmed_unaligned_samples
	cat <(fasta_formatter -i $(word 1,$^) | fakeHaplotype.pl -b) <(fasta_formatter -i $(word 1,$^) | fakeHaplotype.pl -b -a | skipRogers.awk) | trimIntrons.awk > $@
...
parse_sample_introns : $(SAMPLEINTRONMAP) $(INTRONSAMPLES) raw_introns_samples
	...
	parseFASTARecords.pl -f unparsed_introns_samples.fofn -m ../$(word 1,$^) -dddd 2> logs/parseFASTA.stderr > logs/parseFASTA.stdout; \
	cd ..; \
...
unparsed_introns_samples/%.fasta : ../pseudorefs/%.fasta unparsed_introns_samples
	...
	extractIntronsFromGFF3.pl -d -b CDS -i $(word 1,$^) -g ../annotations/$${species}.gff3 -p $* 2> logs/intronome_$*.stderr > $@
...
parse_ref_introns : $(INTRONMAP) $(INTRONREFS) raw_introns_refs
	...
	/usr/bin/time -v parseFASTARecords.pl -f unparsed_introns_refs.fofn -m ../$(word 1,$^) -dddd 2> logs/parseFASTA.stderr > logs/parseFASTA.stdout; \
...
$(INTRONMAP) : $(SCOMAP) $(INTRONREFS)
	...
	/usr/bin/time -v findSingleCopyOrthologIntrons.pl -f raw_introns_refs.fofn -m <(awk 'BEGIN{FS="\t";OFS=FS;}NR==1{gsub("_proteome", "", $$0); print $$0;}NR>1{print $$0;}' $(word 1,$^)) -t $(THRESHOLD) -dd 2> logs/fSCOI.stderr > $@
...
unparsed_introns_refs/%.fasta : ../refs/%.fasta ../annotations/%.gff3 unparsed_introns_refs
	mkdir -p logs; \
	/usr/bin/time -v extractIntronsFromGFF3.pl -d -b CDS -i $(word 1,$^) -g $(word 2,$^) -p $* > $@ 2> logs/intronome_$*.stderr
